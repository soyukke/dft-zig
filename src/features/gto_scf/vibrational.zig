...
const sto3g_data = @import("../basis/sto3g.zig");

fn buildSto3gShells(alloc: std.mem.Allocator, atomic_numbers: []const u8, positions: []const Vec3) ![]ContractedShell {
    var shells_list: std.ArrayList(ContractedShell) = .empty;
    errdefer shells_list.deinit(alloc);

    for (atomic_numbers, 0..) |z, i| {
        const center = positions[i];
        const n_shells = sto3g_data.numShellsForAtom(@intCast(z)) orelse return error.UnsupportedElement;
        const atom_shells = sto3g_data.buildAtomShells(@intCast(z), center) orelse return error.UnsupportedElement;
        for (0..n_shells) |s| {
...
test "H2 STO-3G vibrational frequency" {
    ...
    // H2 at equilibrium (our optimized geometry)
    ...
    var result = try vibrationalAnalysis(
        alloc,
        shells,
        &positions,
        &charges,
    ...
    std.debug.print("  Number of vibrational modes: {d}\n", .{result.vib_frequencies_cm1.len});
    for (result.vib_frequencies_cm1, 0..) |freq, i| {
        std.debug.print("  mode {d}: {d:.2} cm^-1\n", .{ i + 1, freq });
    }
    std.debug.print("  ZPVE: {d:.10} Ha\n", .{result.zpve});

    // PySCF reference: 5480.94 cm⁻¹ (with integer H mass = 1)
    // We should have exactly 1 vibrational mode for diatomic
...
test "H2O STO-3G vibrational frequencies" {
    ...
    // H2O at PySCF equilibrium geometry (close to our optimized)
    var positions = [_]Vec3{
        .{ .x = 0.0, .y = 0.0, .z = 0.0 },
    ...
    std.debug.print("  ZPVE: {d:.10} Ha ({d:.4} kcal/mol)\n", .{ result.zpve, result.zpve * 627.509 });

    // PySCF reference frequencies (cm⁻¹, integer masses):
    //   2169.86 (bend), 4139.43 (symmetric stretch), 4390.48 (asymmetric stretch)
    // ZPVE: 0.0244 Ha (15.30 kcal/mol)
    //
    // Our numerical Hessian accuracy is limited by the ~8e-6 Ha energy gap
    // with PySCF and the use of finite differences. The bending mode is
    // most sensitive to geometry accuracy. Tolerances reflect this.
    try testing.expect(result.vib_frequencies_cm1.len == 3);

    // Sort our frequencies for comparison
    ...
    // Tolerance: 150 cm⁻¹ for bending mode (most sensitive), 100 for stretches
...
test "H2O STO-3G thermodynamic properties" {
    ...
    // Use PySCF optimized geometry (Bohr) for consistency
    var positions = [_]Vec3{
    ...
    var vib_result = try vibrationalAnalysis(
        alloc,
        shells,
        &positions,
        &charges,
    ...
    // Use our SCF energy for the electronic part
    // Run a single-point SCF to get the energy
    var scf_result = try gto_scf.runGeneralRhfScf(
        alloc,
        shells,
        &positions,
    ...
    const thermo = computeThermo(
        ...
        298.15,
        101325,
        2, // sigma = 2 for H2O (C2v)
        true,
    );
    printThermo(&thermo);

    // PySCF reference values (298.15 K, 1 atm):
    //   E_trans  = 0.001416 Ha
    //   S_trans  = 5.515e-5 Ha/K
    //   E_rot    = 0.001416 Ha
    //   S_rot    = 1.701e-5 Ha/K
    //   ZPE      = 0.02437 Ha
    //   S_vib    ≈ 1e-9 Ha/K (negligible)
    //   H_tot    = -74.9377 Ha
    //   G_tot    = -74.9593 Ha
    //   S_tot    = 7.216e-5 Ha/K

    // Verify translational: exact formulas, should match very closely
    try testing.expectApproxEqAbs(0.001416, thermo.e_trans, 1e-5);
    // S_trans has small difference vs PySCF (different CODATA constants)
    try testing.expectApproxEqAbs(5.515e-5, thermo.s_trans, 1e-6);

    // Verify rotational: depends on geometry/masses but formulas are exact
    try testing.expect(!thermo.is_linear);
    try testing.expectApproxEqAbs(0.001416, thermo.e_rot, 1e-5);
    // Rotational entropy: PySCF = 1.701e-5 Ha/K
    // Allow 5% tolerance due to slight geometry differences
    try testing.expectApproxEqAbs(1.701e-5, thermo.s_rot, 1e-6);

    // Verify rot constants match PySCF order of magnitude
    // PySCF: 698.1, 436.2, 268.5 GHz
    std.debug.print("\nRot const comparison vs PySCF:\n", .{});
    std.debug.print("  Ours: {d:.1}, {d:.1}, {d:.1} GHz\n", .{ thermo.rot_const_ghz[0], thermo.rot_const_ghz[1], thermo.rot_const_ghz[2] });
    std.debug.print("  PySCF: 698.1, 436.2, 268.5 GHz\n", .{});

    // Rot constants within 5 GHz (geometry difference)
    ...
    // Total S: PySCF = 7.216e-5 Ha/K
    try testing.expectApproxEqAbs(7.216e-5, thermo.s_tot, 2e-6);

    // H_tot and G_tot: larger tolerance due to frequency differences
    // PySCF H_tot = -74.9377, G_tot = -74.9593
    // Our frequencies differ by up to 120 cm⁻¹ which mainly affects ZPVE/E_vib
...
test "H2 STO-3G thermodynamic properties" {
    ...
    const thermo = computeThermo(
        ...
        298.15,
        101325,
        2, // sigma = 2 for homonuclear diatomic
        true,
    );
    printThermo(&thermo);

    // PySCF reference (H2 at r=1.3459 Bohr, integer masses):
    //   E_elec   = -1.1175 Ha
    //   E_trans  = 0.001416 Ha
    //   S_trans  = 4.475e-5 Ha/K
    //   Rot const: inf, ~1993, ~1993 GHz (geometry-dependent; PySCF at r=1.3513 gives 1976.8)
    //   Linear   = true
    //   E_rot    = 0.000944 Ha (2 DOF for linear)
    //   S_rot    = ~4.57e-6 Ha/K (depends on B)
    //   ZPE      = 0.01254 Ha (our freq is 5503 cm⁻¹ vs PySCF 5481)
    //   H_tot    ≈ -1.1017 Ha
    //   G_tot    ≈ -1.1164 Ha
    ...
    try testing.expectApproxEqAbs(0.001416, thermo.e_trans, 1e-5);
    // S_trans has small difference vs PySCF (different CODATA constants)
    try testing.expectApproxEqAbs(4.471e-5, thermo.s_trans, 1e-6);

    // Rotational (linear: 2 DOF, E_rot = kT)
    try testing.expectApproxEqAbs(0.000944, thermo.e_rot, 1e-5);
    // S_rot differs slightly from PySCF because B differs (geometry)
    try testing.expectApproxEqAbs(4.573e-6, thermo.s_rot, 5e-7);

    // Rot constant B: at r=1.3459 Bohr with integer H mass, B ≈ 1993 GHz
    try testing.expectApproxEqAbs(1993.0, thermo.rot_const_ghz[1], 5.0);

    // Total thermodynamic values (wider tolerance for frequency diff)
    try testing.expectApproxEqAbs(-1.1017, thermo.h_tot, 0.002);
    try testing.expectApproxEqAbs(-1.1164, thermo.g_tot, 0.002);
}
