...
test "DF KS-DFT H2O STO-3G LDA" {
    ...
    std.debug.print("\nH2O STO-3G LDA (DF vs conventional):\n", .{});
    std.debug.print("  Conventional: {d:.10} Ha ({d} iters)\n", .{ result_conv.total_energy, result_conv.iterations });
    std.debug.print("  DF:           {d:.10} Ha ({d} iters)\n", .{ result_df.total_energy, result_df.iterations });
    std.debug.print("  Diff:         {e:.6}\n", .{@abs(result_df.total_energy - result_conv.total_energy)});

    try testing.expect(result_df.converged);
    // DF should match conventional within ~1 mHa
