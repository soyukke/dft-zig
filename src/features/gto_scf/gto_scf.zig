...
test "General RHF H2O 6-31G" {
    const testing = std.testing;
    const alloc = testing.allocator;
    const b631g = @import("../basis/basis631g.zig");

    // H2O geometry in bohr (same as STO-3G test)
    const nuc_positions = [_]math.Vec3{
        .{ .x = 0.0, .y = 0.0, .z = 0.0 }, // O
        .{ .x = 0.0, .y = 1.4305226763, .z = 1.1092692351 }, // H
        .{ .x = 0.0, .y = -1.4305226763, .z = 1.1092692351 }, // H
    ...
    // Build 6-31G shells for H2O
    const o_data = b631g.buildAtomShells(8, nuc_positions[0]).?;
    const h1_data = b631g.buildAtomShells(1, nuc_positions[1]).?;
    const h2_data = b631g.buildAtomShells(1, nuc_positions[2]).?;

    // Combine all shells into a single array
    var all_shells: [b631g.MAX_SHELLS_PER_ATOM * 3]ContractedShell = undefined;
    var count: usize = 0;
    ...
    // Verify basis size: O(9) + H(2) + H(2) = 13
    const n_basis = obara_saika.totalBasisFunctions(shells);
    try testing.expectEqual(@as(usize, 13), n_basis);

    // 10 electrons (O=8, H=1, H=1)
    var result = try runGeneralRhfScf(alloc, shells, &nuc_positions, &nuc_charges, 10, .{});
    defer result.deinit(alloc);

    std.debug.print("\nH2O 6-31G RHF Results:\n", .{});
    std.debug.print("  Total energy:     {d:.10} Ha\n", .{result.total_energy});
    std.debug.print("  Electronic energy: {d:.10} Ha\n", .{result.electronic_energy});
    std.debug.print("  Nuclear repulsion: {d:.10} Ha\n", .{result.nuclear_repulsion});
    std.debug.print("  Iterations:        {d}\n", .{result.iterations});
    ...
    // Reference: PySCF RHF/6-31G H2O = -75.9839484981 Ha
    try testing.expect(result.converged);
    try testing.expectApproxEqAbs(-75.9839484981, result.total_energy, 1e-4);

    // Nuclear repulsion should be the same as STO-3G (same geometry)
...
// test "General RHF H2O 6-31G(2df,p)" {
//     const testing = std.testing;
//     const alloc = testing.allocator;
...
//     const shells = all_shells[0..count];
//
//     // Verify basis size: O(31) + H(5) + H(5) = 41 (Cartesian)
//     const n_basis = obara_saika.totalBasisFunctions(shells);
//     try testing.expectEqual(@as(usize, 41), n_basis);
//
//     // 10 electrons
//     var result = try runGeneralRhfScf(alloc, shells, &nuc_positions, &nuc_charges, 10, .{
//         .use_direct_scf = true,
...
//     defer result.deinit(alloc);
//
//     std.debug.print("\nH2O 6-31G(2df,p) RHF Results (Direct SCF):\n", .{});
//     std.debug.print("  Total energy:     {d:.10} Ha\n", .{result.total_energy});
//     std.debug.print("  Iterations:       {d}\n", .{result.iterations});
//     std.debug.print("  Converged:        {}\n", .{result.converged});
//     std.debug.print("  PySCF (Cartesian): -76.0274095457 Ha\n", .{});
//
//     // Reference: PySCF RHF/6-31G(2df,p) H2O (Cartesian) = -76.0274095457 Ha
//     try testing.expect(result.converged);
//     try testing.expectApproxEqAbs(-76.0274095457, result.total_energy, 1e-3);
// }
