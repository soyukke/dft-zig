...
/// Helper to build STO-3G shells for a set of atoms.
...
test "H2 STO-3G geometry optimization" {
    ...
    // Start from a stretched geometry: 1.8 Bohr (equilibrium ~1.346 Bohr)
    ...
    var result = try optimizeGeometry(
        alloc,
        shells,
        &positions,
        &charges,
    ...
    std.debug.print("  Energy: {d:.12} Ha\n", .{result.energy});
    ...
    std.debug.print("  Bond length: {d:.10} Bohr\n", .{bond_length});

    // PySCF reference: 1.3459 Bohr, E = -1.117506 Ha
...
test "H2O STO-3G geometry optimization" {
    ...
    // Start from approximate experimental geometry (slightly off equilibrium)
    // O at origin, H atoms in y-z plane
    const r_oh: f64 = 1.809; // ~0.957 Angstrom in Bohr
    ...
    std.debug.print("\nH2O optimization result:\n", .{});
    std.debug.print("  Converged: {}\n", .{result.converged});
    std.debug.print("  Steps: {d}\n", .{result.steps});
    std.debug.print("  Energy: {d:.12} Ha\n", .{result.energy});

    // Compute O-H distances and angle
    const oh1 = Vec3{
    ...
    std.debug.print("  O-H1 distance: {d:.10} Bohr\n", .{r1});
    std.debug.print("  O-H2 distance: {d:.10} Bohr\n", .{r2});
    std.debug.print("  H-O-H angle: {d:.4} degrees\n", .{opt_angle});

    // PySCF reference: r_OH = 1.8697 Bohr, angle = 100.03 deg, E = -74.9659 Ha
    try testing.expect(result.converged);
...
test "H2 LDA STO-3G geometry optimization" {
    ...
    // PySCF reference: 1.3921 Bohr, E = -1.121216 Ha
    try testing.expect(result.converged);
...
test "H2O LDA STO-3G geometry optimization" {
    ...
    // PySCF reference: r_OH = 1.9480 Bohr, angle = 96.74 deg, E = -74.7431 Ha
    try testing.expect(result.converged);
...
test "KS-DFT B3LYP geometry optimization H2 STO-3G" {
    ...
    // Initial H2 geometry: R = 1.4 bohr along x-axis
    var positions = [_]Vec3{
        .{ .x = 0.0, .y = 0.0, .z = 0.0 },
        .{ .x = 1.4, .y = 0.0, .z = 0.0 },
    };
    ...
    std.debug.print("\nH2 B3LYP optimization result:\n", .{});
    std.debug.print("  Converged: {}\n", .{result.converged});
    std.debug.print("  Steps: {d}\n", .{result.steps});
    std.debug.print("  Energy: {d:.12} Ha\n", .{result.energy});
    std.debug.print("  Bond length: {d:.10} Bohr\n", .{r_hh});

    // PySCF reference: r_HH = 1.3764 Bohr, E = -1.16554 Ha
    try testing.expect(result.converged);
...
test "KS-DFT B3LYP geometry optimization H2O STO-3G" {
    ...
    // Copy shells to mutable memory since optimizeKsDftGeometry needs a slice
    const shells = try alloc.alloc(ContractedShell, init_shells.len);
    @memcpy(shells, &init_shells);
    defer alloc.free(shells);
    ...
    // PySCF reference: r_OH = 1.9410 Bohr, angle = 97.19 deg, E = -75.3228 Ha
    try testing.expect(result.converged);
