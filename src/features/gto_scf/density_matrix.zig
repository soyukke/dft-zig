...
/// Same as build_density_matrix but writes to an existing buffer.
pub fn update_density_matrix(
    n: usize,
    n_occ: usize,
    c: []const f64,
    p: []f64,
...
pub fn density_rms_diff(n: usize, p_new: []const f64, p_old: []const f64) f64 {
...
test "density matrix 1x1" {
    ...
    const c = [_]f64{0.8};
    const p = try build_density_matrix(alloc, 1, 1, &c);
    defer alloc.free(p);
...
test "density matrix 2x2 H2" {
    ...
    const p = try build_density_matrix(alloc, 2, 1, &c);
    defer alloc.free(p);
...
test "density RMS diff" {
    ...
    const rms = density_rms_diff(2, &p1, &p2);
