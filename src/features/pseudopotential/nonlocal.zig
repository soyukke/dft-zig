...
pub fn sphericalBessel(l: i32, x: f64) f64 {
    const ax = @abs(x);
    if (ax < 1e-6) return sphericalBesselSmall(l, x);
    ...
    return sphericalBesselMiller(l, x);
}
...
fn sphericalBesselMiller(l: i32, x: f64) f64 {
    ...
    if (nlim > max_nlim) {
        ...
        return sphericalBesselDirect(l, x);
    }
...
fn sphericalBesselDirect(l: i32, x: f64) f64 {
...
fn sphericalBesselSmall(l: i32, x: f64) f64 {
    const x2 = x * x;
    switch (l) {
...
pub fn realSphericalHarmonic(l: i32, m: i32, x: f64, y: f64, z: f64) f64 {
    ...
    return switch (l) {
        0 => realYlm0(m),
        1 => realYlm1(m, nx, ny, nz),
        2 => realYlm2(m, nx, ny, nz),
        3 => realYlm3(m, nx, ny, nz),
        4 => realYlm4(m, nx, ny, nz),
        else => 0.0,
    };
}

/// Y_0^0 = 1/sqrt(4π)
fn realYlm0(m: i32) f64 {
...
/// Y_1^0 = sqrt(3/4π) * z
/// Y_1^1 = sqrt(3/4π) * x
fn realYlm1(m: i32, nx: f64, ny: f64, nz: f64) f64 {
...
fn realYlm2(m: i32, nx: f64, ny: f64, nz: f64) f64 {
...
fn realYlm3(m: i32, nx: f64, ny: f64, nz: f64) f64 {
...
fn realYlm4(m: i32, nx: f64, ny: f64, nz: f64) f64 {
    const pi = std.math.pi;
    const nz2 = nz * nz;
    const nx2 = nx * nx;
    const ny2 = ny * ny;
...
pub fn numMValues(l: i32) usize {
    return @intCast(2 * l + 1);
}

/// Pre-computed lookup table for radialProjector as a function of |g|.
...
pub const RadialTable = struct {
    ...
    pub fn init(
        ...
        for (0..n) |i| {
            ...
            values[i] = radialProjector(beta, r, rab, l, g);
        }
    ...
    pub fn evalDeriv(self: *const RadialTable, g: f64) f64 {
        if (g < self.dg) {
