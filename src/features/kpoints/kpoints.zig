...
fn log_kpoint_info(io: std.Io, comptime fmt: []const u8, args: anytype) !void {
    const logger = runtime_logging.stderr(io, .info);
    try logger.print(.info, fmt, args);
}

fn log_kpoint_warn(io: std.Io, msg: []const u8) !void {
...
pub fn generate_kmesh(
    ...
    return mesh.generate_kmesh(alloc, kmesh, recip, shift);
}
...
pub fn generate_kmesh_symmetry(
    ...
    const full = try mesh.generate_kmesh(alloc, kmesh, recip, shift);
    errdefer alloc.free(full);

    const ops = try symmetry.get_symmetry_ops(alloc, cell, atoms, 1e-6);
    defer alloc.free(ops);

    if (ops.len == 0) return full;

    const filtered_ops = try reduction.filter_sym_ops_for_kmesh(alloc, ops, kmesh, shift, 1e-8);
    ...
    try log_kpoint_info(io, "scf: symmetry ops {d}\n", .{ops.len});

    const reduced = try reduction.reduce_kmesh(
        alloc,
        kmesh,
        shift,
        filtered_ops,
    ...
    const verified = try reduction.verify_kmesh_reduction(
        alloc,
        full,
        reduced,
        kmesh,
    ...
    if (!verified) {
        try log_kpoint_warn(io, "scf: kpoint reduction failed verification; using full mesh");
        alloc.free(reduced);
        return full;
    }

    if (reduced.len < full.len) {
        try log_kpoint_info(io, "scf: kpoints reduced {d} -> {d}\n", .{ full.len, reduced.len });
    }
...
test "abinit kmesh reduction for aluminum fcc" {
    ...
    const kpoints = try generate_kmesh_symmetry(
        alloc,
        std.testing.io,
        .{ 6, 6, 6 },
        shift,
