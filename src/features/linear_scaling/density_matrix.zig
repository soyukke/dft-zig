...
pub fn mc_weeny(
    ...
    while (iter < iterations) : (iter += 1) {
        ...
        const next = try sparse.add_scaled(alloc, d2, 3.0, d3, -2.0, threshold);
...
pub fn mc_weeny_non_orthogonal(
    alloc: std.mem.Allocator,
    initial: sparse.CsrMatrix,
    ...
    while (iter < iterations) : (iter += 1) {
        ...
        const next = try sparse.add_scaled(alloc, psp, 3.0, psp_sp, -2.0, threshold);
        current.deinit(alloc);
        current = next;
    }
    return current;
...
pub fn trace_overlap(density: sparse.CsrMatrix, overlap: sparse.CsrMatrix) !f64 {
    return sparse.trace_product(density, overlap);
}

pub fn normalize_trace(matrix: *sparse.CsrMatrix, target: f64) !void {
    if (target <= 0.0) return error.InvalidTraceTarget;
    const current = sparse.trace(matrix.*);
    if (current == 0.0) return error.ZeroTrace;
    sparse.scale_in_place(matrix, target / current);
}

pub fn normalize_trace_overlap(
    matrix: *sparse.CsrMatrix,
    overlap: sparse.CsrMatrix,
    target: f64,
) !void {
    if (target <= 0.0) return error.InvalidTraceTarget;
    const current = try trace_overlap(matrix.*, overlap);
    if (current == 0.0) return error.ZeroTrace;
    sparse.scale_in_place(matrix, target / current);
}

pub fn density_from_hamiltonian(
    alloc: std.mem.Allocator,
    ...
    const diag = try sparse.diagonal_values(alloc, hamiltonian);
    ...
    if (denom <= 1e-12) {
        ...
        try normalize_trace_overlap(&density, overlap, electrons);
        return density;
    }
    ...
    var p0 = try sparse.add_scaled(alloc, diag_mat, 1.0, hamiltonian, scale, threshold);
    defer p0.deinit(alloc);

    var density = try mc_weeny_non_orthogonal(alloc, p0, overlap, iterations, threshold);
    errdefer density.deinit(alloc);
    try normalize_trace_overlap(&density, overlap, electrons);
    return density;
}

test "mc_weeny keeps identity" {
    const alloc = std.testing.allocator;
    const triplets = [_]sparse.Triplet{
        .{ .row = 0, .col = 0, .value = 1.0 },
        .{ .row = 1, .col = 1, .value = 1.0 },
    };
    var identity = try sparse.CsrMatrix.init_from_triplets(alloc, 2, 2, triplets[0..]);
    defer identity.deinit(alloc);

    var purified = try mc_weeny(alloc, identity, 2, 0.0);
    defer purified.deinit(alloc);

    try std.testing.expectApproxEqAbs(@as(f64, 1.0), purified.value_at(0, 0), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 1.0), purified.value_at(1, 1), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 0.0), purified.value_at(0, 1), 1e-12);
}

test "mc_weeny_non_orthogonal matches orthogonal when overlap is identity" {
    const alloc = std.testing.allocator;
    ...
    var initial = try sparse.CsrMatrix.init_from_triplets(alloc, 2, 2, triplets[0..]);
    defer initial.deinit(alloc);

    var ortho = try mc_weeny(alloc, initial, 1, 0.0);
    defer ortho.deinit(alloc);

    var non_ortho = try mc_weeny_non_orthogonal(alloc, initial, overlap, 1, 0.0);
    defer non_ortho.deinit(alloc);

    try std.testing.expectApproxEqAbs(ortho.value_at(0, 0), non_ortho.value_at(0, 0), 1e-12);
    try std.testing.expectApproxEqAbs(ortho.value_at(1, 1), non_ortho.value_at(1, 1), 1e-12);
}

test "normalize_trace rescales density" {
    const alloc = std.testing.allocator;
    var diag = try sparse.diagonal(alloc, 2, 1.0);
    defer diag.deinit(alloc);

    try normalize_trace(&diag, 4.0);
    try std.testing.expectApproxEqAbs(@as(f64, 4.0), sparse.trace(diag), 1e-12);
}

test "normalize_trace_overlap rescales using overlap" {
    const alloc = std.testing.allocator;
    ...
    try normalize_trace_overlap(&density, overlap, 2.0);
    const value = try trace_overlap(density, overlap);
    try std.testing.expectApproxEqAbs(@as(f64, 2.0), value, 1e-12);
}

test "density_from_hamiltonian normalizes trace" {
    const alloc = std.testing.allocator;
    const h_triplets = [_]sparse.Triplet{
        .{ .row = 0, .col = 0, .value = 0.0 },
        .{ .row = 1, .col = 1, .value = 1.0 },
    };
    var h = try sparse.CsrMatrix.init_from_triplets(alloc, 2, 2, h_triplets[0..]);
    defer h.deinit(alloc);

    var s = try sparse.diagonal(alloc, 2, 1.0);
    defer s.deinit(alloc);

    var density = try density_from_hamiltonian(alloc, h, s, 1.0, 2, 0.0);
    defer density.deinit(alloc);

    const trace_val = try trace_overlap(density, s);
