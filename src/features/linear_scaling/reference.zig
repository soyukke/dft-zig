...
pub fn compareReference(reference: ReferenceData, candidate: ReferenceData) !ComparisonReport {
    const energy = compare.compareScalar(reference.energy, candidate.energy);
    const density = try compare.compareSlices(reference.density, candidate.density);
    return .{ .energy = energy, .density = density };
}

test "compareReference computes energy and density metrics" {
...
test "compareReference detects mismatched density length" {
    ...
    try std.testing.expectError(error.MismatchedLength, compareReference(ref, cand));
}
