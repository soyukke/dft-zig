...
fn fft_real_to_reciprocal_rfft(
    ...
    expand_rfft_output(data, rfft_out, nx, ny, nz, nx_c);
    ...
    const out = try alloc.alloc(math.Complex, total);
    remap_fft_data(grid, data, out);
    return out;
}

fn expand_rfft_output(
    data: []math.Complex,
    rfft_out: []const fft.LibComplex,
    nx: usize,
    ny: usize,
    nz: usize,
    nx_c: usize,
...
fn remap_fft_data(grid: Grid, data: []const math.Complex, out: []math.Complex) void {
    const nx = grid.nx;
    const ny = grid.ny;
    const nz = grid.nz;
    var idx: usize = 0;
