...
pub fn cutoff_radius(cell: math.Mat3) f64 {
...
/// build_potential_grid for periodic systems.
pub fn cutoff_coulomb_kernel(g2: f64, g_mag: f64, r_cut: f64) f64 {
    if (g2 < 1e-12) {
...
pub fn cutoff_coulomb_energy_kernel(g2: f64, g_mag: f64, r_cut: f64) f64 {
    // Same kernel as potential — energy just multiplies by Ω/2 |ρ(G)|²
    return cutoff_coulomb_kernel(g2, g_mag, r_cut);
}
...
test "cutoff Coulomb kernel G=0 limit" {
    const testing = std.testing;
    const r_cut = 10.0;
    const v0 = cutoff_coulomb_kernel(0.0, 0.0, r_cut);
    // G=0 limit: 4π R_c²
...
test "cutoff Coulomb kernel reduces to 8pi/G2 for large Rc" {
    ...
    const v = cutoff_coulomb_kernel(g2, g_mag, r_cut);
...
test "direct ion-ion energy H2" {
    ...
    const e = direct_ion_ion_energy(&charges, &positions);
...
test "direct ion-ion forces finite difference" {
    ...
    const forces = try direct_ion_ion_forces(alloc, &charges, &positions);
    ...
    pos_p[0].x += delta;
    pos_m[0].x -= delta;
    const ep = direct_ion_ion_energy(&charges, &pos_p);
    const em = direct_ion_ion_energy(&charges, &pos_m);
