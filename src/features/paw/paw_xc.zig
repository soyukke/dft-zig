...
/// Find Q_ij^{L=0}(r) entry from paw.qijl for given (i,j) pair.
fn findQijL0(paw: PawData, i: usize, j: usize) ?[]const f64 {
    return findQijL(paw, i, j, 0);
}
...
/// Compute on-site XC correction to D_ij for one PAW atom.
///
/// The correct PAW on-site D^xc is the functional derivative of E_onsite w.r.t. ρ_ij:
///   D^xc_ij = ∫ V_xc(ρ^AE + core^AE) × φ_i^AE × φ_j^AE × rab dr
///           - ∫ V_xc(ρ^PS + core^PS) × φ_i^PS × φ_j^PS × rab dr
///
/// where ρ^AE uses AE partial waves and ρ^PS uses PS partial waves.
pub fn computePawDijXc(
    alloc: std.mem.Allocator,
    dij_xc: []f64,
    paw: PawData,
    rhoij: []const f64,
    ...
    const vxc_ae = try alloc.alloc(f64, n_mesh);
    defer alloc.free(vxc_ae);
    const vxc_ps = try alloc.alloc(f64, n_mesh);
    defer alloc.free(vxc_ps);

    @memset(rho_ae, 0.0);
    @memset(rho_ps, 0.0);
    const four_pi = 4.0 * std.math.pi;

    // Build AE on-site density from AE partial waves
    for (0..nbeta) |i| {
        const ae_i = paw.ae_wfc[i].values;
        for (0..nbeta) |j| {
            const rij = rhoij[i * nbeta + j];
            if (@abs(rij) < 1e-30) continue;
            const ae_j = paw.ae_wfc[j].values;
            const n_r = @min(n_mesh, @min(ae_i.len, ae_j.len));
            for (0..n_r) |k| {
                if (r[k] < 1e-10) continue;
                const r2 = r[k] * r[k];
                rho_ae[k] += rij * ae_i[k] * ae_j[k] / (four_pi * r2);
            }
        }
    }

    // Build PS on-site density from PS partial waves
    for (0..nbeta) |i| {
        const ps_i = paw.ps_wfc[i].values;
        for (0..nbeta) |j| {
            const rij = rhoij[i * nbeta + j];
            if (@abs(rij) < 1e-30) continue;
            const ps_j = paw.ps_wfc[j].values;
            const n_r = @min(n_mesh, @min(ps_i.len, ps_j.len));
            for (0..n_r) |k| {
                if (r[k] < 1e-10) continue;
                const r2 = r[k] * r[k];
                rho_ps[k] += rij * ps_i[k] * ps_j[k] / (four_pi * r2);
            }
        }
    }

    // Evaluate V_xc at (ρ^AE + core^AE) and (ρ^PS + core^PS)
    for (0..n_mesh) |k| {
        var rho_with_ae_core = rho_ae[k];
        var rho_with_ps_core = rho_ps[k];
        if (rho_core_ae) |core_ae| {
            if (k < core_ae.len) rho_with_ae_core += core_ae[k];
        }
        if (rho_core_ps) |core_ps| {
            if (k < core_ps.len) rho_with_ps_core += core_ps[k];
        }

        const n_ae = @max(rho_with_ae_core, 1e-30);
        const eval_ae = xc.evalPoint(xc_func, n_ae, 0.0);
        vxc_ae[k] = eval_ae.df_dn;

        const n_ps = @max(rho_with_ps_core, 1e-30);
        const eval_ps = xc.evalPoint(xc_func, n_ps, 0.0);
        vxc_ps[k] = eval_ps.df_dn;
    }

    // D^xc_ij = ∫ V_xc(AE) × φ_i^AE × φ_j^AE dr - ∫ V_xc(PS) × φ_i^PS × φ_j^PS dr
    for (0..nbeta) |i| {
        for (0..nbeta) |j| {
            const ae_i = paw.ae_wfc[i].values;
            const ae_j = paw.ae_wfc[j].values;
            ...
            var sum: f64 = 0.0;
            for (0..n_r) |k| {
                const ae_contrib = vxc_ae[k] * ae_i[k] * ae_j[k];
                const ps_contrib = vxc_ps[k] * ps_i[k] * ps_j[k];
                sum += (ae_contrib - ps_contrib) * rab[k] * ctrapWeight(k, n_r);
            }
            dij_xc[i * nbeta + j] = sum;
        }
    }
}

/// Compute on-site XC energy correction for one PAW atom.
///
/// The correct PAW on-site XC energy is:
///   E_xc_onsite = E_xc[ρ^AE + core^AE] - E_xc[ρ^PS + core^PS]
///
/// where ρ^AE uses AE partial waves and ρ^PS uses PS partial waves.
pub fn computePawExcOnsite(
    alloc: std.mem.Allocator,
    paw: PawData,
    rhoij: []const f64,
    r: []const f64,
    ...
    // E_xc = ∫ f_xc(ρ^AE + core^AE) × 4πr² dr - ∫ f_xc(ρ^PS + core^PS) × 4πr² dr
    var exc_ae: f64 = 0.0;
    var exc_ps: f64 = 0.0;
    for (0..n_mesh) |k| {
        const r2 = r[k] * r[k];
        ...
        const n_ae = @max(rho_with_ae_core, 1e-30);
        const eval_ae = xc.evalPoint(xc_func, n_ae, 0.0);
        exc_ae += eval_ae.f * four_pi * r2 * rab[k] * ctrapWeight(k, n_mesh);

        const n_ps = @max(rho_with_ps_core, 1e-30);
        const eval_ps = xc.evalPoint(xc_func, n_ps, 0.0);
        exc_ps += eval_ps.f * four_pi * r2 * rab[k] * ctrapWeight(k, n_mesh);
    }

    return exc_ae - exc_ps;
}

/// Compute on-site Hartree D_ij for one PAW atom.
///
/// For pseudized augmentation charges (PSQ), E_H^AE ≠ E_H^{PS+Q},
/// giving a non-zero D^H contribution. PS Hartree density includes Q_ij^{L=0}.
/// XC is computed separately without Q (L=0-only approximation requires this
/// because the nonlinear XC functional mixes L-channels that we don't have).
pub fn computePawDijHartree(
    alloc: std.mem.Allocator,
    dij_h: []f64,
    paw: PawData,
    rhoij: []const f64,
    ...
    const rho_ps = try alloc.alloc(f64, n_mesh);
    defer alloc.free(rho_ps);
    const vh_ae = try alloc.alloc(f64, n_mesh);
    defer alloc.free(vh_ae);
    const vh_ps = try alloc.alloc(f64, n_mesh);
    defer alloc.free(vh_ps);

    const four_pi = 4.0 * std.math.pi;
    @memset(rho_ae, 0.0);
    @memset(rho_ps, 0.0);

    // Build AE on-site density (no Q, no core)
    for (0..nbeta) |i| {
        const ae_i = paw.ae_wfc[i].values;
        for (0..nbeta) |j| {
            const rij = rhoij[i * nbeta + j];
            ...
            for (0..n_r) |k| {
                if (r[k] < 1e-10) continue;
                rho_ae[k] += rij * ae_i[k] * ae_j[k] / (four_pi * r[k] * r[k]);
            }
        }
    }

    // Build PS on-site density with Q_ij^{L=0} (no core)
    for (0..nbeta) |i| {
        const ps_i = paw.ps_wfc[i].values;
        for (0..nbeta) |j| {
            const rij = rhoij[i * nbeta + j];
            ...
            for (0..n_r) |k| {
                if (r[k] < 1e-10) continue;
                rho_ps[k] += rij * ps_i[k] * ps_j[k] / (four_pi * r[k] * r[k]);
            }
            if (findQijL0(paw, i, j)) |q_vals| {
                const n_q = @min(n_mesh, q_vals.len);
                for (0..n_q) |k| {
                    if (r[k] < 1e-10) continue;
                    rho_ps[k] += rij * q_vals[k] / (four_pi * r[k] * r[k]);
                }
            }
        }
    }

    // Compute Hartree potential for AE and PS (L=0 only)
    radialHartreePotentialL0(r, rab, rho_ae, vh_ae, n_mesh);
    radialHartreePotentialL0(r, rab, rho_ps, vh_ps, n_mesh);

    // D^H_ij = ∫ V_H^AE × ae_i × ae_j dr - ∫ V_H^PS × (ps_i × ps_j + Q_ij) dr
    for (0..nbeta) |i| {
        for (0..nbeta) |j| {
            const ae_i = paw.ae_wfc[i].values;
            const ae_j = paw.ae_wfc[j].values;
            ...
            var sum: f64 = 0.0;
            for (0..n_r) |k| {
                const ae_contrib = vh_ae[k] * ae_i[k] * ae_j[k];
                var ps_contrib = vh_ps[k] * ps_i[k] * ps_j[k];
                if (findQijL0(paw, i, j)) |q_vals| {
                    if (k < q_vals.len) {
                        ps_contrib += vh_ps[k] * q_vals[k];
                    }
                }
                sum += (ae_contrib - ps_contrib) * rab[k] * ctrapWeight(k, n_r);
            }
...
/// Compute on-site Hartree energy E_H^AE - E_H^{PS+Q} for one PAW atom.
pub fn computePawEhOnsite(
    alloc: std.mem.Allocator,
    paw: PawData,
    rhoij: []const f64,
    r: []const f64,
    ...
    // Build PS on-site density with Q
    for (0..nbeta) |i| {
        const ps_i = paw.ps_wfc[i].values;
        for (0..nbeta) |j| {
            const rij = rhoij[i * nbeta + j];
    ...
    for (0..n_mesh) |k| {
        const r2 = r[k] * r[k];
        const w = rab[k] * ctrapWeight(k, n_mesh);
        eh_ae += 0.5 * vh_ae[k] * rho_ae[k] * four_pi * r2 * w;
        eh_ps += 0.5 * vh_ps[k] * rho_ps[k] * four_pi * r2 * w;
    }
...
// =============================================================================
// Angular-resolved PAW on-site XC (for GGA functionals)
// =============================================================================

const lebedev = @import("../grid/lebedev.zig");
const nonlocal = @import("../pseudopotential/nonlocal.zig");
const LebedevPoint = lebedev.LebedevPoint;
