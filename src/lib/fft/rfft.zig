...
pub const RealPlan = struct {
    ...
    pub fn forward(self: *const RealPlan, real_input: []const f64, complex_output: []Complex) void {
        ...
        // Step 1: Pack real data as complex z[k] = x[2k] + i*x[2k+1].
        for (0..half_n) |k| {
        ...
        // The full DFT X[k] = E[k] + W_N^k * O[k]; X[N/2-k] = conj(E[k]) + W_N^{N/2-k}*conj(O[k]).
        // Save Z values before overwriting (process pairs symmetrically)
        const z0 = complex_output[0];

        // X[0] is purely real: X[0] = E[0] + O[0]; X[N/2] = E[0] - O[0].
    ...
    pub fn inverse(self: *const RealPlan, complex_input: []const Complex, real_output: []f64) void {
        ...
        // with E[mid], O[mid] real, giving E[mid] = X[mid].re, O[mid] = -X[mid].im.
        if (half_n % 2 == 0) {
            const mid = half_n / 2;
        ...
        // Step 3: Unpack to real. After IFFT, z contains interleaved real values
        // via z[k] = x[2k] + i*x[2k+1]; real_output aliases the same memory.
    }
};
