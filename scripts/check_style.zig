...
//!
//! File directives:
//!     // style-ignore: <rule>[, <rule>...]
//!                          Disable rule(s) for a single file. The directive
//!                          must appear in the leading comment block.
//!
//! Rules:
...
fn applyFileIgnores(cfg: *Config, content: []const u8) void {
    ...
    while (lines.next()) |raw| {
        ...
        const marker = mem.indexOf(u8, line, "style-ignore:") orelse continue;
        var rules = mem.tokenizeAny(u8, line[marker + "style-ignore:".len ..], ", ");
        while (rules.next()) |name| {
            if (parseRule(name)) |rule| cfg.disable(rule);
        }
    }
}
...
fn checkAst(
    ...
    if (want_fn_len or want_precedence) {
        try checkNodes(allocator, cfg, counter, path, content, &tree, want_fn_len, want_precedence);
    }
    if (want_defer) {
...
fn checkNodes(
    ...
    const FnRange = struct {
        line_opening: usize,
        line_closing: usize,
        body_line_opening: usize,
        body_line_closing: usize,
    };
    ...
    for (tags, datas, 0..) |tag, data, node_usize| {
        ...
        if (want_fn_len and tag == .fn_decl) {
            ...
            const token_body_opening = tree.firstToken(body_node);
            const token_body_closing = tree.lastToken(body_node);
            const line_opening = tree.tokenLocation(0, token_opening).line;
            const line_closing = tree.tokenLocation(0, token_closing).line;
            const body_line_opening = tree.tokenLocation(0, token_body_opening).line;
            const body_line_closing = tree.tokenLocation(0, token_body_closing).line;
            try functions.append(allocator, .{
                .line_opening = line_opening,
                .line_closing = line_closing,
                .body_line_opening = body_line_opening,
                .body_line_closing = body_line_closing,
            });
        }
    ...
    for (functions.items, 0..) |fn_range, i| {
        ...
        const function_length = countMeaningfulFunctionLines(
            content,
            fn_range.body_line_opening,
            fn_range.body_line_closing,
        );
...
fn countMeaningfulFunctionLines(content: []const u8, line_opening: usize, line_closing: usize) usize {
    ...
    while (i < content.len) : (i += 1) {
        if (content[i] != '\n') continue;
        if (line_index >= line_opening and line_index <= line_closing) {
            if (isMeaningfulFunctionLine(content[start..i])) count += 1;
        }
        line_index += 1;
        start = i + 1;
    }
    if (start < content.len and line_index >= line_opening and line_index <= line_closing) {
        if (isMeaningfulFunctionLine(content[start..])) count += 1;
    }
    return count;
}

fn isMeaningfulFunctionLine(raw: []const u8) bool {
    const line = mem.trim(u8, raw, " \t\r");
    if (line.len == 0) return false;
    if (mem.startsWith(u8, line, "//")) return false;
    if (onlyDelimiterTokens(line)) return false;
    return true;
}

fn onlyDelimiterTokens(line: []const u8) bool {
    for (line) |c| {
        switch (c) {
            ' ', '\t', '{', '}', '(', ')', '[', ']', ';', ',' => {},
            else => return false,
...
fn checkFile(
    ...
    var file_cfg = cfg.*;
    applyFileIgnores(&file_cfg, content);

    if (file_cfg.isEnabled(.missing_final_newline) and content[content.len - 1] != '\n') {
    ...
    while (i < content.len) : (i += 1) {
        if (content[i] == '\n') {
            try checkLine(&file_cfg, counter, path, content[start..i]);
            start = i + 1;
        }
    }
    if (start < content.len) {
        try checkLine(&file_cfg, counter, path, content[start..]);
    }

    try checkAst(allocator, &file_cfg, counter, path, content);
}
