#!/usr/bin/env python3
"""Compare Cu band structures: DFT-Zig vs ABINIT (metal)."""
...
NBANDS = 16
NELEC = 19
NOCC = 10  # ceil(19/2) = 10 occupied bands
...
def load_dft_zig_fermi():
    """Load DFT-Zig Fermi energy from SCF output."""
    fermi_file = 'out_dft_zig/scf_eigenvalues.csv'
    if not os.path.exists(fermi_file):
        return None
    # Read fermi_energy from the output
    info_file = 'out_dft_zig/scf_info.txt'
        ...
        with open(info_file) as f:
            for line in f:
                if 'fermi' in line.lower():
                    parts = line.split('=')
                    if len(parts) >= 2:
                        return float(parts[1].strip()) * Ry_to_eV
    return None
...
def load_abinit_fermi():
    ...
    with open(abo_file) as f:
        content = f.read()
    # Search for Fermi energy in ABINIT output
    match = re.search(r'Fermi.*energy.*?(-?[\d.]+)\s*Ha', content)
    if match:
...
def calculate_kpath_distances(kpts):
    ...
    # Cu FCC primitive cell: a/2 * [(0,1,1), (1,0,1), (1,1,0)]
    # a = 3.615 Angstrom, a/2 = 1.8075 Angstrom
    a = 1.8075
    a1 = np.array([0.0, a, a])
    a2 = np.array([a, 0.0, a])
    a3 = np.array([a, a, 0.0])
...
def estimate_fermi_energy(bands, nocc):
    """Estimate Fermi energy as midpoint between highest occupied and lowest unoccupied at each k."""
    nkpts = len(bands[0])
    # For metals: Fermi energy ~ max of highest occupied band
    # (since bands cross Fermi level, use max of band nocc-1)
    return max(bands[nocc - 1])
...
def plot_comparison():
    ...
    # Metal: align to Fermi energy (estimated from highest occupied band)
    dft_ef = estimate_fermi_energy(dft_bands, NOCC)
    for band in dft_bands:
        for i in range(len(band)):
            band[i] -= dft_ef

    abinit_ef = estimate_fermi_energy(abinit_bands, NOCC)
    ...
    plt.suptitle('Cu Band Structure: DFT-Zig vs ABINIT', fontsize=14)
    plt.tight_layout()
    ...
    ax.set_title('DFT-Zig - ABINIT (Cu)')
    ...
    # Print summary
    print("\n" + "=" * 50)
    print("Summary (Cu - metal)")
    print("=" * 50)
    print(f"DFT-Zig E_F (estimated): {dft_ef:.4f} eV", end='')
    if dft_time is not None:
        print(f"  ({dft_time:.1f}s)")
    else:
        print()
    print(f"ABINIT E_F (estimated):  {abinit_ef:.4f} eV", end='')
    if abinit_time is not None:
...
def run_regression_check():
    ...
    MSE_THRESHOLDS = {
        0: 50,       # 3s (current: 21)
        1: 400,      # 3s (current: 155)
        2: 400,      # 3p (current: 154)
        3: 400,      # 3p (current: 154)
        4: 800,      # 3p (current: 389)
        5: 2000,     # 3d (current: 921)
        6: 2000,     # 3d (current: 986)
        7: 2500,     # 3d (current: 1103)
        8: 2500,     # 3d (current: 1193)
        9: 1200,     # 3d (current: 528)
        10: 200,     # 4s/4p (current: 63)
        11: 200,     # conduction (current: 73)
        12: 200,     # conduction (current: 79)
        13: 200,     # conduction (current: 84)
        14: 400,     # conduction (current: 169)
        15: 2000,    # conduction (current: 985)
    }
    ...
    # Metal: align to Fermi energy
    dft_ef = estimate_fermi_energy(dft_bands, NOCC)
    dft_bands_aligned = [[e - dft_ef for e in band] for band in dft_bands]

    abinit_ef = estimate_fermi_energy(abinit_bands, NOCC)
    abinit_bands_aligned = [[e - abinit_ef for e in band] for band in abinit_bands]
    ...
    # Report
    print("\n" + "=" * 50)
    print("Regression Check Results (Cu - metal)")
    print("=" * 50)
