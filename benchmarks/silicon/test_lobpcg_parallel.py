#!/usr/bin/env python3
"""Test LOBPCG parallel execution produces identical results to serial."""
...
# Must be larger than LOBPCG convergence tolerance (iterative_tol = 1e-8)
# Serial and parallel may converge through slightly different paths
TOLERANCE = 1e-6
...
def compare_results(serial_dir, parallel_dir):
    """Compare band energies between serial and parallel LOBPCG runs."""
    ...
    print("=" * 50)
    print("LOBPCG Parallel vs Serial Comparison")
    print("=" * 50)
    ...
    if failed_checks == 0:
        print("\nPASS: LOBPCG parallel execution produces identical results!")
        return True
    else:
...
def main():
    ...
    serial_config = "dft_zig_lobpcg_test_serial.toml"
    parallel_config = "dft_zig_lobpcg_test_parallel.toml"
    serial_dir = "out_lobpcg_serial"
    parallel_dir = "out_lobpcg_parallel"

    if run_serial:
        print("=" * 50)
        print("Running Serial LOBPCG (lobpcg_parallel=false)")
        print("=" * 50)
        run_dft_zig(serial_config)

    if run_parallel:
        print("=" * 50)
        print("Running Parallel LOBPCG (lobpcg_parallel=true)")
        print("=" * 50)
